...
class Order:
    ...
    def order_info(self):   #получение строки с информацией о заказе
...
class Courier:  #класс курьера
    ...
    def cancel_order(self, text):
        DB = DataBase("db.sqlite3")
        DB.courier_comment(self.Ord.number, text)
        DB.change_order_status(self.Ord.number, 1)  # изменение статуса взятого заказа на выполняющийся
        DB.close()
...
def logout(message):    #/logout
    if message.chat.id in courier_list and courier_list[message.chat.id].login_status == True:
        bot.send_message(message.chat.id, "{} вышел из системы".format(courier_list[message.chat.id].login))
...
def get_new_order(message):
    if courier_list[message.chat.id].order_status == True:
        ...
        info = courier_list[message.chat.id].Ord.order_info()
        ...
        if courier_list[message.chat.id].new_order():
            ...
            info, locations = courier_list[message.chat.id].Ord.order_info()
            bot.send_message(message.chat.id, info)
...
def check_answer(message):
    if message.chat.id not in courier_list or courier_list[message.chat.id].state == "no":
        ...
        if message.text == "Подтверждение":
            ...
            print("{} подтвердил получение заказа №{}.".format(courier_list[message.chat.id].login, courier_list[message.chat.id].Ord.number))
        ...
        courier_list[message.chat.id].cancel(message.text)
